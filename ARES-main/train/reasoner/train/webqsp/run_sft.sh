...
DEFAULT_CONFIG="${REASONER_ROOT}/configs/llamafactory/webqsp/llama31_8b_answer_sft.yaml"
if [ "$#" -ge 1 ]; then
  CONFIG_PATH="$1"
  shift
else
...
RESOLVED_CONFIG="${RESOLVED_CONFIG_DIR}/llama31_8b_answer_sft.webqsp.resolved.yaml"
python3 "${REASONER_ROOT}/scripts/resolve_llamafactory_config.py" "${CONFIG_PATH}" "${RESOLVED_CONFIG}"

LOG_DIR="${ARES_ROOT}/checkpoint/webqsp/kgqa_answer/llama31_8b_sft/logs"
mkdir -p "${LOG_DIR}"
...
export SFT_LOG_FILE
export WANDB_PROJECT="${WANDB_PROJECT:-kgqa_answer_webqsp}"
export WANDB_NAME="${WANDB_NAME:-llama31-8b-answer-sft}"
export WANDB_DIR="${WANDB_DIR:-${ARES_ROOT}/checkpoint/webqsp/kgqa_answer/llama31_8b_sft/wandb}"
