...
def eval_hit_any(prediction, answer, double_check):
    if len(prediction) == 0:
        return 0
    for pred in prediction:
        for a in answer:
            if match(pred, a):
                ...
                if match(a, pred.split('ans:')[-1].strip()):
                    return 1
    return 0
...
def _build_default_hal_stats() -> Dict[str, Any]:
    return {
        'g_no_ans': 0,
        'g_c': 0,
        'g_w': 0,
...
def uses_double_check(question: str) -> bool:
    question = (question or "").lower()
    return any(
        keyword in question
        for keyword in ['when', 'what year', 'which year', 'where', 'sport', 'what countr', 'language', 'nba finals', 'world series']
    )


def normalize_answer_list(answer: Sequence[str], question: str = "") -> list[str]:
    normalized = sorted(remove_duplicates(list(answer or [])), key=len, reverse=True)
    question_lower = (question or "").lower()
    if 'when' in question_lower or 'what year' in question_lower:
        for idx in range(len(normalized)):
            if '-' in normalized[idx] and normalized[idx].split('-')[0].isdigit():
                normalized[idx] = normalized[idx].split('-')[0]
    return normalized


def evaluate_prediction_core(
    prediction_text: str,
    answer: Sequence[str],
    question: str,
    split: Optional[str] = None,
) -> Dict[str, Any]:
    parsed_prediction = get_pred(prediction_text, split)
    normalized_answer = normalize_answer_list(answer, question=question)
    double_check = uses_double_check(question)
    if split == '\n':
        no_answer = len(parsed_prediction) == 0
    else:
        prediction_lower = (prediction_text or '').lower()
        no_answer = (
            len(parsed_prediction) == 0
            or 'ans:' not in prediction_lower
            or 'ans: not available' in prediction_lower
            or 'ans: no information available' in prediction_lower
        )
    precision_score, matched_pred, num_pred = eval_precision(parsed_prediction, normalized_answer, double_check)
    recall_score, matched_gold, num_gold = eval_recall(parsed_prediction, normalized_answer, double_check)
    f1_score = eval_f1(precision_score, recall_score)
    hit = eval_hit(parsed_prediction, normalized_answer, double_check)
    hit_any = eval_hit_any(parsed_prediction, normalized_answer, double_check)
    exact_match = float(f1_score == 1.0)
    totally_wrong = float(recall_score == 0.0)
    answer_format_valid = float(not no_answer)
    return {
        'prediction_lines': parsed_prediction,
        'ground_truth': normalized_answer,
...
def eval_single_prediction_corrected(
    prediction_text: str,
    answer: Sequence[str],
    question: str,
    split: Optional[str] = None,
    retrieved_triplets: Optional[Sequence[Sequence[str]]] = None,
    a_entity_in_graph: Optional[bool] = None,
) -> Dict[str, Any]:
    core = evaluate_prediction_core(prediction_text=prediction_text, answer=answer, question=question, split=split)
    hal_score = None
    hal_stats = None
    if retrieved_triplets is not None and a_entity_in_graph is not None:
        hal_stats = _build_default_hal_stats()
        subgraph_ent = get_all_retrieved_entities(retrieved_triplets)
        hal_score, hal_stats = eval_hal_score(
            core['prediction_lines'],
            core['ground_truth'],
            core['double_check'],
            bool(a_entity_in_graph),
            bool(core['no_answer']),
            subgraph_ent,
            hal_stats,
        )
        hal_score = (hal_score + 1.5) / (1 + 1.5) * 100
    return {
        **core,
        'hal_score': hal_score,
        'hal_stats': hal_stats,
...
def eval_results(predict_file, cal_f1=True, split=None, subset=False, bad_samples=False, eval_hops=-1):
    ...
    with open(predict_file, 'r') as f, open(detailed_eval_file, 'w') as f2:
        for line in tqdm(f):
            ...
            if cal_f1:
                ...
                hit_any = eval_hit_any(prediction, answer, double_check)
                ...
                hit_list.append(hit)
                hit_any_list.append(hit_any)
                acc_list.append(recall_score)
                f2.write(json.dumps({'id': id, 'prediction': prediction, 'ground_truth': answer, 'hit': hit, 'hit_any': hit_any, 'f1': f1_score, 'precision': precision_score, 'recall': recall_score, 'hal_score': hal_score}) + '\n')
            else:
                raise NotImplementedError
    if len(hit_list) == 0:
        null_out = [0] * 13
    ...
    avg_hit_any = sum(hit_any_list) * 100 / len(hit_any_list)
    ...
    result_str = f"Hit@1: {avg_hit}, Hitnew: {avg_hit_any}, Macro F1: {avg_f1}, Macro Precision: {avg_precision}, Macro Recall: {avg_recall}, Exact Match: {num_exact_match}, Totally Wrong: {num_totally_wrong}, Hal Score: {avg_hal_score}"
    print(result_str)
    ...
    with open(eval_result_path, 'w') as f:
        f.write(result_str)
    return avg_hit, avg_hit_any, avg_f1, avg_precision, avg_recall, num_exact_match, num_totally_wrong, micro_f1, micro_precision, micro_recall, total_cnt, no_ans_cnt, no_ans_cnt / total_cnt, avg_hal_score, stats
